...
exports.run = (Service, Data, args,) => {
    Service.User.getUserByUID(Service.session.uid, (err, user) => {
        ...
        if (Service.cache.addons.chat.locked[Service.session.channel]) {
            ...
            Service.Utils.Server.broadcast(`The server chat has been unlocked by ${user.username}#${user.tag}`, Service.io, Service.session.channel, Service.sessions)
        } else {
            ...
            Service.Utils.Server.broadcast(`The server chat has been locked by ${user.username}#${user.tag}`, Service.io, Service.session.channel, Service.sessions)
        }
    })
}
